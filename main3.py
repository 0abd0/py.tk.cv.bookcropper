...
def crop_and_split_image(image_path, output_folder, top_pixels, bottom_pixels, left_pixels, right_pixels, split_image, split_position, auto_crop, filename_affix):
    ...
    if split_image and split_position:
        ...
        if split_position < width:
            ...
            # Save the split images with affixes
            left_output_path = os.path.join(output_folder, f'{name}{filename_affix}_left{ext}')
            right_output_path = os.path.join(output_folder, f'{name}{filename_affix}_right{ext}')
        ...
        # Save the single cropped/trimmed image with affix
        output_path = os.path.join(output_folder, f'{name}{filename_affix}{ext}')
...
def process_images():
    ...
    filename_affix = filename_affix_var.get()
    ...
    for filename in os.listdir(input_folder_path):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            crop_and_split_image(image_path, output_folder_path, top_pixels, bottom_pixels, left_pixels, right_pixels, split_image, split_position, auto_crop, filename_affix)
...
filename_affix_var = tk.StringVar()
...
tk.Label(app, text="Filename Affix (e.g., '_processed')").pack(pady=10)
tk.Entry(app, textvariable=filename_affix_var, width=20).pack(pady=5)
